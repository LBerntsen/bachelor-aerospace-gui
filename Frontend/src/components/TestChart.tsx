      ...
      <div className="pt-16 h-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            ...
            <XAxis dataKey="timeStamp" type="number" domain={['dataMin', 'dataMax']} tickFormatter={(time) => new Date(time).toLocaleTimeString()} label={{value: "Time", position: "insideBottom",offset: -5}}/>
            <YAxis label={{value: sensorId, angle: -90, position: "insideLeft"}}/>
            <Tooltip labelFormatter={(time) => new Date(time).toLocaleTimeString()}/>
            <CartesianGrid stroke="#1e293b" vertical={false}/>
            <Line type="monotone" dataKey="value" stroke="#22d3ee" dot={false} isAnimationActive={false}/>
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
